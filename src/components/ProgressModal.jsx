export default function ProgressModal({ title, current, total, label, onCancel }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-4">
      <div className="rounded-2xl border p-8 w-full max-w-sm text-center" style={{ background: "#111", borderColor: "#2a2a2a" }}>
        ...
        <p className="text-sm text-gray-400 mb-5">{label || (done ? "Complete!" : `${current} of ${total}`)}</p>
        ...
        <div className="text-xs text-gray-500">{done ? "All done!" : `Processing ${current} of ${total}...`}</div>
        {!done && onCancel && (
          <button onClick={onCancel} className="mt-4 text-xs text-gray-600 hover:text-gray-400 underline">Cancel</button>
        )}
      </div>
    </div>
  );
